...
extension UIView {
  ...
  @discardableResult
  func _anchorEqualWidth(_ width: NSLayoutDimension, multiplier: CGFloat = 1.0, priority: UILayoutPriority = .required) -> NSLayoutConstraint {
    translatesAutoresizingMaskIntoConstraints = false
    
    let anchor = widthAnchor.constraint(equalTo: width, multiplier: multiplier)
    anchor.priority = priority
    anchor.isActive = true
    return anchor
  }
  ...
  @discardableResult
  func _anchorCenterX(_ centerX: NSLayoutXAxisAnchor, _ constant: CGFloat, relation: NSLayoutConstraint.Relation = .equal, priority: UILayoutPriority = .required) -> NSLayoutConstraint {
    translatesAutoresizingMaskIntoConstraints = false
    
    var anchor: NSLayoutConstraint
    switch relation {
    case .greaterThanOrEqual:
      anchor = centerXAnchor.constraint(greaterThanOrEqualTo: centerX, constant: -constant)
    case .lessThanOrEqual:
      anchor = centerXAnchor.constraint(lessThanOrEqualTo: centerX, constant: -constant)
    default:
